...
select
    ...
    name as 이름,
    age as 나이,
    if(age between 20 and 29, '20대', '20대가 아님') as age_flag, /* 알리어스(Alias) = 별칭 */
    1 as num
from
	student_study_mst;
...
/*
	1. case
    case 
		when 조건 then 조건이 참일때 결과
		when 조건2가소 then 조건 2가 참일때 결과
		else 위의 모든 조건이 아닐때 결과
	end
*/
...
select distinct /* distinct 결과에서 중복을 제거해준다 */
